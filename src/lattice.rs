...
pub struct Lattice<T, B: Boundary<usize>> {
    array: Box<[T]>,
    pub bound: B
}

impl<T: Default + Copy, B: Boundary<usize>> Lattice<T, B> {
...
impl<T, B: Boundary<usize>> Lattice<T, B> {
...
impl<T: Copy, B: Boundary<usize>> Lattice<T, B> {
...
impl<T, B: Boundary<usize>> Index<Pos2D<usize>> for Lattice<T, B> {
    type Output = T;
...
impl<T, B: Boundary<usize>> Index<(usize, usize)> for Lattice<T, B> {
    type Output = T;
...
impl<T, B: Boundary<usize>> IndexMut<Pos2D<usize>> for Lattice<T, B> {
    fn index_mut(&mut self, pos: Pos2D<usize>) -> &mut Self::Output {
        &mut self.array[pos.row_major(self.height())]
    }
}
impl<T, B: Boundary<usize>> IndexMut<(usize, usize)> for Lattice<T, B> {
