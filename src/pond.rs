...
pub struct Pond {
    pub env: Environment<Cell<Grn<5, 6>>, NeighbourhoodType, BoundaryType>,
...
impl Pond {
    pub fn new(
        env: Environment<Cell<Grn<5, 6>>, NeighbourhoodType, BoundaryType>,
            ...
            for spin in new_spins {
                ...
                if let LatticeEntity::SomeCell(cell) = self.env.cells.get_entity_mut(spin) {
                    for i in 1..cell.genome.input_signals.len() {
                        if cell.genome.nth_output_gene(i + 1).active {
