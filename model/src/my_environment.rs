...
impl MyEnvironment {
    ...
    pub fn update_chem_gradient(&mut self) {
        for pos in self.chem_lattice.iter_positions() {
            ...
            if let Spin::Some(cell_index) = self.env.cell_lattice[pos] {
                let prev_chem = self.chem_lattice[pos];
                let rel_cell = &mut self.env.cells[cell_index];
    ...
    pub fn draw_solid_target(&mut self) {
        ...
        for j in 0..theight {
            for i in 0..twidth {
                ...
                let Some(valid_pos) = self.env.bounds.lattice_boundary.valid_pos(trans_pos) else {
                    continue;
                };
