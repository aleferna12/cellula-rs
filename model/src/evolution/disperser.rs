use rand_xoshiro::Xoshiro256StarStar;
use crate::pond::Pond;
use cellulars_lib::constants::CellIndex;
use rustworkx_core::connectivity::connected_components;
use crate::evolution::selector::{IndexedFit, Selector, WeightedOrderedSelection};

pub trait Disperser {
    fn disperse(&mut self, dispersable: &[Pond]) -> Vec<DispersionEvent>;
}

#[derive(Debug)]
pub struct DispersionEvent {
    pub from: usize,
    pub to: usize,
    pub indexes: Vec<CellIndex>,
}

pub struct SelectiveDispersion {
    pub selector: WeightedOrderedSelection<Xoshiro256StarStar>,
}

impl SelectiveDispersion {
    /// Returns at most `n_props` 
    pub fn get_prop_indexes(&self, pond: &Pond, n_props: usize) -> Vec<Vec<CellIndex>> {
        if n_props < 1 {
            return vec![];
        }
        
        let neighs_graph = pond.env.build_neighbours_graph(pond.env.cell_search_scaler);
        let mut subgraphs = connected_components(&neighs_graph);
        // There is only one cluster
        if subgraphs.len() <= 1 {
            return vec![];
        }

        subgraphs.sort_by(|subgraph1, subgraph2| { 
            subgraph1.len().cmp(&subgraph2.len()) 
        });
        
        subgraphs[0..n_props.min(subgraphs.len() - 1)]
            .iter()
            .map(|subgraph| {
                subgraph.iter().map(|&index| neighs_graph[index]).collect()
            })
            .collect()
    }
}

impl Disperser for SelectiveDispersion {
    fn disperse(&mut self, dispersable: &[Pond]) -> Vec<DispersionEvent> {
        let indexed_disp: Vec<_> = dispersable.iter()
            .enumerate()
            .map(|(i, disp)| IndexedFit {
                index: i,
                fit: disp
            })
            .collect();
        let selected = self.selector.select(indexed_disp.as_slice());
        let mut prop_counts = vec![0usize; selected.len()];
        for &s in &selected {
            prop_counts[s.index] += 1;
        }
        let mut props: Vec<_> = prop_counts.into_iter()
            .enumerate()
            .map(|(i, count)| self.get_prop_indexes(&dispersable[i], count.saturating_sub(1)))
            .collect();
        
        selected.into_iter()
            .enumerate()
            .filter_map(|(to, from)| {
                if from.index == to {
                    None
                } else {
                    props[from.index].pop().map(|prop| DispersionEvent {
                        from: from.index,
                        to,
                        indexes: prop
                    })
                }
            })
            .collect()
    }
}