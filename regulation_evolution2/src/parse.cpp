...
unsigned int ugetpar(FILE *fp, const char *parameter, unsigned int default_val, bool wrapflag) {
    char *token;
    int value;
    ...
    if (token == 0) {
        /* default value */
        warning("No token %s found. Using default value %u.\n", parameter, default_val);
        return default_val;
    }
    /* read it */
...
double *dgetparlist(FILE *fp, const char *parameter, int n, bool wrapflag) {
    ...
    if (i < n) {
        ...
        fprintf(stdout, "Full array is ");
        for (int i = 0; i < n; i++) {
            fprintf(stdout, " %lf ", value[i]);
        }
        fprintf(stdout, "\n");
    }
