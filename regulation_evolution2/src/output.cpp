...
int makeDirIfNeeded(const char *dirname, bool existing_dir, bool replace_dir) {
    struct stat sb{};
    bool makedir = true;
    if (stat(dirname, &sb) == 0 && S_ISDIR(sb.st_mode)) {
        ...
        if (replace_dir) {
            makedir = true;
            if (filesystem::remove_all(
                    filesystem::path(string(dirname))
                    ) == static_cast<std::uintmax_t>(-1))
                throw runtime_error("Could not delete directory: " + string (dirname));
        }
    }
    if (makedir) {
