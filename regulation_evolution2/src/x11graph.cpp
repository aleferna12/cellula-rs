...
void X11Graphics::EndScene(void) {
    ...
    if (store && !(thetime++ % par.save_movie_period))
        StoreCompPict();
...
void X11Graphics::ReadColorTable(XColor *cols, const char *colortable_filename) {
    ...
    if ((fpc = fopen(name, "r")) == NULL) {
        ...
        snprintf(message, 999, "X11Graphics::ReadColorTable: Colormap '%s' not found.", name);
        //cerr << message << endl;
        throw (message);
...
void X11Graphics::MakeColorMap() {
    ...
    ReadColorTable(colors, par.colortablefile.c_str());
...
#endif
    //shm=0;
    printf("save_movie %d, graphics %d\n", store, par.graphics);
    printf("depth/8 = %d\n", depth / 8);
    if (pseudoCol8) {
...
void X11Graphics::Write(char *fname, int quality) {
    ...
    if (!png_colors) {
        ...
        cout << "First call - inside Write(), making colors" << endl;

        ReadColorTable(png_colors, par.colortablefile.c_str());
        cout << "read color table" << endl;
