...
* gain control over the local density of the generated mesh. The
* meshing algorithm calls these functions to decide if a certain
* triangle must be refined.
*
...
/** \brief Reproducible results
 *
 * By default the algorithm generates reproducible triangulations. If
 * not required you can disable that with \p bReproducibleResults=false
 * to save time.
 *
 * @note Reproducible results are created \em inside \p pZone. If you
 * need reproducable results outside also then create a global zone and
 * call unifyGrid(0) once.
 */
	bool bReproducibleResults;

/** \brief Low quality but high performance grid meshing
 *
 * By default bGridMeshingLQ=false in order to create high quality
 * triangles everywhere. By using \p bGridMeshingLQ=true you can
 * save time but a few bad shaped triangles may remain. This
 * option works only together with grid-meshing.
 */
	bool bGridMeshingLQ;
