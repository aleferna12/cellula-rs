...
*
* @param [out] vZones Vector that will store pointers to all existing Zone2 objects.
*/
 ...
 * @param [out] vSegmentsOut: Output segments
  * @param [in] offTolerance is used to prevent truncation of segments that
 * are slightly outside the convex hull which might be the case if your input
 * segments are subject to tiny rounding errors. If this value is >0, then
 * segment endpoints which are slightly outside the convex hull of the
 * triangulation are inserted into the triangulation.
  * @param [in] bKeepOffSegments Specifies if segments outside the triangulation
 * shall be kept, default: false
 *
 ...
 * \endif
 *
 * @note Draping segments onto a TIN does not insert them. Use Fade_2D::createConstraint() for that purpose.
 */
